...
function extractValue(arr, key) {
  let result = arr.map(function (obj, i, arr) {
    return obj[key];
  });
  return result;
}
...
//   strArray.filter(function (char, i, arr) {
...
function vowelCount(str) {
  const vowels = "aeiou";
  return Array.from(str.toLowerCase()).reduce(function (
    accumulator,
    currentValue
  ) {
  ...
  let result = arr.reduce(
    function (sum, currentValue) {
      if (callback(currentValue)) {
        sum[0].push(currentValue);
      } else {
        sum[1].push(currentValue);
      }
      return sum;
    },
    [[], []]
